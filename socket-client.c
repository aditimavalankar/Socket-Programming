...
char* commandcheck(char *recvBuff, int connfd,int type)
{
	...
	{
		system("ls -l client1/ --time-style=+%Y%m%d%H%M%S -t | cut -d' ' -f5,6,7,8,9 > server/ls");
		fflush(stdout);
		int x;
		x=fopen("server/ls","r");
		fgets(str, 500, x);
...
//			puts(command[2]);
	...
	{
		system("ls -l client1/ --time-style=+%Y%m%d%H%M%S -t | cut -d' ' -f6,7,8,9 > server/ls2");
		fflush(stdout);
		int x;
	...
	else if(strcmp(command[0], "FileDownload")==0)
	{
		...
		system("ls");
		printf("*%s*",command[1]);
		if((fi=open(command[1],O_RDONLY))<0)
			perror("can't open");
		int index=0;
	//	while((ch=read(fi)!=EOF)
	//		otp[index++]=ch;
		read(fi,otp,1024);
		//otp[index++]='\0';
